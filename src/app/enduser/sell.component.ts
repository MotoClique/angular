...
export class AppSell implements OnInit {
    ...
    variant_term: string = '';
	  ...
	  ngAfterViewInit(){
        ...
        this.disabled = {save: false};
              ...
              if(that.sharedService.sharedObj.postItem && that.sharedService.sharedObj.postItem.sell_id == id){
							...
							that.commonService.enduserService.getSell("",id,"",null,null,null)
							  .subscribe( data => {
    ...
    onPrdModelSelect(evt,prdModel){
		  ...
		  this.commonService.enduserService.getAddress(this.userDetail.user_id,"")
				.subscribe( result => {
					...
					if(that.addresses.length && that.addresses.length>1){
						...
						that.router.navigate(['/Container/Sell','blank','create']);
						/*that.detail = true;
						that.editMode = true;
						that.screenMode = {add:true, edit:false};
						that.dynamicFormComponent.generateField("Sell");
	  ...
	  onAddressSelect(evt,add){
		...
		this.router.navigate(['/Container/Sell','blank','create']);
		/*this.detail = true;
		this.editMode = true;
		this.screenMode = {add:true, edit:false};
		this.dynamicFormComponent.generateField("Sell");
	  ...
	  onEdit(evt){
		...
		this.router.navigate(['/Container/Sell',this.item.sell_id,'edit']);
	  }
	  ...
	  displayItem(){
		...
		this.done = true;
		this.router.navigate(['/Container/Sell',this.item.sell_id]);
	  }
	...
	loadSellSet(){
		var that = this;
		that.loading = true;
		this.commonService.enduserService.getSell(this.userDetail.user_id,"","",this.params.count,this.params.skip,this.params.limit)
			 .subscribe( data => {
				if(!(that.results) || that.results.length<=0){
					  that.lastScroll = 0;
					  that.noMoreData = false;
